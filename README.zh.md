...
DSH Web UI 分支徽标插件：在**输入框工具行左端**显示当前工作区文件夹的 git 分支，支持查看、切换、新建、重命名、删除分支。
...
然后**重启 `dsh web`**，输入框工具行左端出现 `⎇ 分支名` 徽标即安装成功；点击徽标，分支面板**向上弹出**。
...
- 点击徽标弹出分支面板（从输入框**向上展开**）：顶部输入框新建；列表点击行切换、行尾 ✎ 重命名、行尾 🗑 删除（需确认）；底部"刷新"。
...
3. 跑一遍冒烟测试（`node scripts/smoke.mjs`，用临时仓库验证全部操作）；
4. `npm version patch && npm publish`，然后 `git push`；使用者重新执行上面的 add 命令 + 重启即升级。
...
- **Client 半边**（`src/client/index.js`）：注册进 `conversation.input.left` 槽位，通过 `fetch` 调宿主路由；样式为内联 `<style data-plugin>` 注入，全部使用 DSH 主题 token。
